...
from .networks import MLP, ColdMLP, default_init, ensemblize
    ...
    def __call__(
        ...
        if self.gc_encoder is not None:
            ...
            state_action = self.sale_state_action(
                jnp.concatenate([observations], axis=-1)
            )
...
class GCBilinearModelBasedValue(nn.Module):
    ...
    def setup(self):
        ...
        activ_fn = getattr(nn, self.activ_fn_name)
        self.norm = LnActiv(activation=activ_fn)
        self.norm_goals = LnActiv(activation=activ_fn)
        # self.zs_dropout = nn.Dropout(rate=0.5)
        # self.zsa_dropout = nn.Dropout(rate=0.5)
...
def compute_encoder_loss_core(
    ...
    def loop_body(carry_pred_zs, i):
        ...
        next_predicted_latent_state = encoder_module_def.apply(
            ...
            carry_pred_zs,
            action_step,
            method=ModelBasedEncoder.model_all,
        )
        step_dyn_loss = jnp.mean(
            jnp.square(next_predicted_latent_state - target_zs_step)
        )
...
def compute_dino_style_encoder_loss_core(
    ...
    def loop_body(carry_pred_zs_continuous, i):
        ...
        reward_logits_step = encoder_module_def.apply(
            {"params": online_encoder_params},
            carry_pred_zs_continuous,
            method=ModelBasedEncoder.get_rewards,
        )
        ...
        step_reward_loss = binary_cross_entropy_from_logits(
            logits=reward_logits_step, labels=rewards[:, i]
        )
