...
def main(_):
    ...
    config_flags.DEFINE_config_file(
        "agent", f"impls/agents/{FLAGS.algo_name}.py", lock_config=False
    )
    ...
    config = FLAGS.agent
    config["frame_stack"] = (
        FLAGS.frame_stack if FLAGS.frame_stack > 0 else None
    )  # Override from CLI
    config["alpha"] = FLAGS.alpha
    env, train_dataset, val_dataset = make_env_and_datasets(
        FLAGS.env_name, frame_stack=None, dataset_dir=FLAGS.dataset_dir
    ...
    while i <= FLAGS.train_steps:
        ...
        if i % FLAGS.log_interval == 0:
            ...
            if val_dataset is not None:
                ...
                val_batch["observations"] = jnp.reshape(
                    val_batch["observations"], (batch_size, -1)
                )
                val_batch["next_observations"] = jnp.reshape(
                    batch["next_observations"], (batch_size, -1)
                )
                val_batch["actions"] = jnp.reshape(
                    val_batch["actions"], (batch_size, -1)
                )

                _, encoder_info = agent._encoder_loss_fn_for_grad(
                    agent.encoder.params, val_batch, i
                )
        ...
        if i % FLAGS.eval_interval == 0:  # or i == 1
            if FLAGS.eval_on_cpu:
                ...
                eval_agent = jax.device_put(
                    agent, device=jax.devices("gpu")[0]
                )  # agent
            renders = []
            eval_metrics = {}
